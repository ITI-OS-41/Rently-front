...
export default function StoreForm(props) {
  ...
  return (
    <>
      <h5 className={classes.title} style={{ textAlign: 'center'}}>Store Information</h5>
      <Formik
        onSubmit={(values) => {
          submitForm(values)
        }}
          ...
          return (
            <form onSubmit={handleSubmit}>
              <Grid container spacing={2}>
                ...
                {(imagePreview || values.photo) && (<img src={imagePreview || values.photo || ''} className={imageClasses} style={{objectFit: 'cover', height: '100px', width: '100px'}}/>)}
                </Grid>
                ...
                <Grid item xs={12} style={{textAlign: 'center'}}>
                  <Button
                    ...
                    variant="contained"
                    color="primary"
                    onClick={()=>{document.getElementById('photo').click()}}>Upload image</Button>
                </Grid>

                <Grid item xs={12}>
                  <input id="photo" name="photo" type="file" hidden onChange={(event) => {
                ...
                <Grid item xs={12}>
                  <TextField variant="outlined" fullWidth multiline rowsMax={8} id="description" name="description" label="description" value={values.description} onBlur={handleBlur}
                             onChange={handleChange} error={touched.description && Boolean(errors.description)} helperText={touched.description && errors.description}/>
                </Grid>
